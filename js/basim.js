...
function simInit() {
	...
	simRunnerSpawns = document.getElementById(HTML_RUNNER_SPAWNS);
...
function simStartStopButtonOnClick() {
	if (simIsRunning) {
		...
		let runnerSpawns = simParseSpawnsInput(simRunnerSpawns);
		let healerSpawns = simParseSpawnsInput(simHealerSpawns);
		if (movements === null) {
...
function baInit(maxRunnersAlive, totalRunners, maxHealersAlive, totalHealers, runnerMovements, runnerSpawns, healerSpawns) {
	baRunners = [];
	baRunnersToRemove = [];
	baRunnerSpawns = runnerSpawns;
	baRunnerSpawnsIndex = 0;
	baRunnersAlive = 0;
	baRunnersKilled = 0;
	baHealersAlive = 0;
	baHealersKilled = 0;
...
function baTick() {
	...
	if (baTickCounter > 1 && baTickCounter % 10 === 1) {
		if (baRunnersAlive < baMaxRunnersAlive && baRunnersKilled + baRunnersAlive < baTotalRunners && baRunnerSpawns.length === 0) {
			let movements;
	...
	// custom spawn times
	if (baRunnersAlive < baMaxRunnersAlive && baRunnersKilled + baRunnersAlive < baTotalRunners && baRunnerSpawns.length > 0) {
		if (baRunnerSpawns[baRunnerSpawnsIndex] === baTickCounter){
			let movements;
			if (baRunnerMovements.length > baRunnerMovementsIndex) {
				movements = baRunnerMovements[baRunnerMovementsIndex++];
			} else {
...
function drawLogs() {
	...
	if (!logZone.some(item => item.logType === "North")) {
		if (mCurrentMap === mWAVE_1_TO_9)
			mAddItem(new lLog(baWAVE1_NORTH_LOG_X, baWAVE1_NORTH_LOG_Y, "North", ++simCurrentFoodId));
		else
			mAddItem(new lLog(baWAVE10_NORTH_LOG_X, baWAVE10_NORTH_LOG_Y, "North", ++simCurrentFoodId));
	}
	if (!logZone.some(item => item.logType === "South")) {
		if (mCurrentMap === mWAVE_1_TO_9)
			mAddItem(new lLog(baWAVE1_SOUTH_LOG_X, baWAVE1_SOUTH_LOG_Y, "South", ++simCurrentFoodId));
		else 
			mAddItem(new lLog(baWAVE10_SOUTH_LOG_X, baWAVE10_SOUTH_LOG_Y, "South", ++simCurrentFoodId));
	}
